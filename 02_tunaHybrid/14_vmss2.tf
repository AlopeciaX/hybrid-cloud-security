resource "azurerm_linux_virtual_machine_scale_set" "vmss2" {
  ...
  network_interface {
    ...
    ip_configuration {
      name      = "vmss2-ip-config"
      primary   = true
      subnet_id = azurerm_subnet.vnet2_vmss.id

      application_gateway_backend_address_pool_ids = [
        "${azurerm_application_gateway.appgw2.id}/backendAddressPools/vmss2-backend-pool"
      ]
    }
  }
  boot_diagnostics {
  ...
  depends_on = [azurerm_application_gateway.appgw2]
}

resource "azurerm_monitor_autoscale_setting" "vmss2_autoscale" {
  name = "tuna-vmss2-autoscale"
  resource_group_name = var.rgname
  location            = var.loca2
